...
export function start() {
    ...
    angular.module('testApp', [])
        .directive('appDirective', ['appService', '$document', function(appService, $document) {
            ...
            return {
                template: '<div ng-repeat="command in commands">{{command}}</div>' +
                    '<div><table><tr><td>' + inputText + '</td><td>' +
                    '<form ng-submit="execCommand()"><input type="text" ' +
                    'ng-model="data.currentCommand" /></form></td></tr></table></div>',
                link: function(scope) {
                    scope.commands = [];
